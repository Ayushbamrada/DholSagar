    ...
    fun onSubmit() {
        viewModelScope.launch {
            ...
            try {
                ...
                val portfolioImageUrlsDeferred = _portfolioImageUris.value.map { uri -> async { providerRepository.uploadFile(uri, "portfolio_images/$uid") } }
                val portfolioVideoUrlDeferred = _portfolioVideoUri.value?.let { uri -> async { providerRepository.uploadFile(uri, "portfolio_videos/$uid") } }
                val kycDocUrlsDeferred = _kycDocumentUris.value.map { (docId, uri) -> async { docId to providerRepository.uploadFile(uri, "kyc_documents/$uid") } }
                val portfolioImageResults = portfolioImageUrlsDeferred.awaitAll()
                val portfolioVideoResult = portfolioVideoUrlDeferred?.await()
                val kycDocResults = kycDocUrlsDeferred.awaitAll()
